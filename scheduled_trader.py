...
DEFAULT_TOP_ALLOCATE_COUNT = int(os.getenv('TOP_ALLOCATE_COUNT', '3'))
TRADE_COOLDOWN_PATH = os.path.join('data', 'trade_cooldowns.json')
COOLDOWN_MINUTES = float(os.getenv('TRADE_COOLDOWN_MINUTES', '30'))
EARNINGS_BLACKOUT_HOURS = float(os.getenv('EARNINGS_BLACKOUT_HOURS', '24'))
RISK_PER_TRADE = float(os.getenv('RISK_PER_TRADE', '0.01'))
ATR_STOP_MULT = float(os.getenv('TRADE_ATR_STOP_MULT', '1.0'))
ATR_TP_MULT = float(os.getenv('TRADE_ATR_TP_MULT', '2.0'))
...
def _get_extended_hours_limit_pct() -> float:
    try:
        pct = float(os.getenv('EXTENDED_HOURS_LIMIT_PCT', '0.01'))
    except Exception:
        pct = 0.01
    if pct < 0:
        pct = abs(pct)
...
def invest_flow():
    ...
    top_n = int(os.getenv('INVEST_TOP_N', '1'))
    invest_total_pct = float(os.getenv('INVEST_TOTAL_PCT', '0.01'))
...
def analyze_mag7_and_invest(reallocate_full_arg=None, place_order_arg=None, invest_yes_arg=None, order_executor=None):
    ...
    try:
        allocate_count = int(os.getenv('TOP_ALLOCATE_COUNT', str(DEFAULT_TOP_ALLOCATE_COUNT)))
    except Exception:
